...
# Script parameters
outfile   = 'davinci_prog_mlp.bin'
outCfile  = 'davinci_prog_mlp.c'
cprogname = 'prog_mlp'
expfile   = 'davinci_prog_mlp_exp.bin'         # expected output in $readmemb() format
...
vregR1 = 2      # VV-Engine register to store MAC result
vregR2 = 3      # VV-Engine register to store MAC result
...
as_addComment('Start of Compute')
...
# 13VBADD Latency: Start
vv_shiftOff()       # disable all shifting
for i in range(13):  # Emulate addition of 13 MAC results, for 784 input vector 
    vv_add(vregB, vregR1)       # Add bias-vector
    vv_mov(vregR2, VVREG.O)     # R2 = tanh(M2@V2 + B2)
# Add bias vector
vv_add(vregB, vregR2)       # Add bias-vector
# Apply ReLU
